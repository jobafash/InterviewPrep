...
Sorting the array will take O(N∗logN)O(N* logN)O(N∗logN). Overall, the function will take O(N∗logN+N2)O(N * logN + N^2)O(N∗logN+N​2​​), which is asymptotically equivalent to O(N2)O(N^2)O(N​2​​).
Space complexity#
