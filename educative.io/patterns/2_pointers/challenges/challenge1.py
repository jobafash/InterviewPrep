...
Sorting the array will take O(N∗logN)O(N*logN)O(N∗logN). Overall searchQuadruplets() will take O(N∗logN+N3)O(N * logN + N^3)O(N∗logN+N​3​​), which is asymptotically equivalent to O(N3)O(N^3)O(N​3​​).
Space complexity#

The space complexity of the above algorithm will be O(N)O(N)O(N) which is required for sorting.
