...
Time and space complexity Time Complexity of the above solution will be O(n)O(n)O(n) and space complexity will also be O(n)O(n)O(n).
...
main()
'''
Time Complexity: Time complexity of this solution is O(n)O(n)O(n) as we iterate through all numbers of the input once.
