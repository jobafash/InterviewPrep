...
The time complexity of the above algorithm is O(n+k)O(n + k)O(n+k), as the last two for loops will run for O(n)O(n)O(n) and O(k)O(k)O(k) times respectively.
Space complexity#

The algorithm needs O(k)O(k)O(k) space to store the extraNumbers.
