...
The algorithm runs in constant space O(1)O(1)O(1) but modifies the input array.
Similar Problems#

Problem 1: Can we solve the above problem in O(1)O(1)O(1) space and without modifying the input array?
...
'''
The time complexity of the above algorithm is O(n)O(n)O(n) and the space complexity is O(1)O(1)O(1).
'''