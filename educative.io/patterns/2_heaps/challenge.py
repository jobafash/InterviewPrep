...
A brute force solution could be to take one interval at a time and go through all the other intervals to find the next interval. This algorithm will take O(N2)O(N^2)O(N​2​​) where ‘N’ is the total number of intervals. Can we do better than that?
...
The time complexity of our algorithm will be O(NlogN),O(NlogN),O(NlogN), where ‘N’ is the total number of intervals.
Space complexity#

The space complexity will be O(N)O(N)O(N) because we will be storing all the intervals in the heaps.
