...
All the additional space used by our algorithm is for the output list. Since, at most, we will have a total of O(2N)O(2^N)O(2​N​​) subsets, and each subset can take up to O(N)O(N)O(N) space, therefore, the space complexity of our algorithm will be O(N∗2N)O(N*2^N)O(N∗2​N​​).
