...
This algorithm will run most of the times in O(logN)O(logN)O(logN). However, since we only skip two numbers in case of duplicates instead of half of the numbers, the worst case time complexity will become O(N)O(N)O(N).
Space complexity ##
