...
Given ‘N’ ropes, we need O(N∗logN)O(N*logN)O(N∗logN) to insert all the ropes in the heap. In each step, while processing the heap, we take out two elements from the heap and insert one. This means we will have a total of ‘N’ steps, having a total time complexity of O(N∗logN)O(N*logN)O(N∗logN).
Space complexity#

The space complexity will be O(N)O(N)O(N) because we need to store all the ropes in the heap.
