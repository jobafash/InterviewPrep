...
A brute force solution could be to sort the array and return the largest K numbers. The time complexity of such an algorithm will be O(N∗logN)O(N*logN)O(N∗logN) as we need to use a sorting algorithm like Timsort if we use Java’s Collection.sort(). Can we do better than that?
...
This will ensure that we always have ‘K’ largest numbers in the heap. The most efficient way to repeatedly find the smallest number among a set of numbers will be to use a min-heap. As we know, we can find the smallest number in a min-heap in constant time O(1)O(1)O(1), since the smallest number is always at the root of the heap. Extracting the smallest number from a min-heap will take O(logN)O(logN)O(logN) (if the heap has ‘N’ elements) as the heap needs to readjust after the removal of an element.
...
As discussed above, it will take us O(logK)O(logK)O(logK) to extract the minimum number from the min-heap. So the overall time complexity of our algorithm will be O(K∗logK+(N−K)∗logK)O(K*logK+(N-K)*logK)O(K∗logK+(N−K)∗logK) since, first, we insert ‘K’ numbers in the heap and then iterate through the remaining numbers and at every step, in the worst case, we need to extract the minimum number and insert a new number in the heap. This algorithm is better than O(N∗logN)O(N*logN)O(N∗logN).
...
As discussed above, the time complexity of this algorithm is O(K∗logK+(N−K)∗logK)O(K*logK+(N-K)*logK)O(K∗logK+(N−K)∗logK), which is asymptotically equal to O(N∗logK)O(N*logK)O(N∗logK)
Space complexity#

The space complexity will be O(K)O(K)O(K) since we need to store the top ‘K’ numbers in the heap.
