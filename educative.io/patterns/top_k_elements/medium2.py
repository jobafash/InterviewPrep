...
The time complexity of the above algorithm is O(D∗logD)O(D*logD)O(D∗logD) where ‘D’ is the number of distinct characters in the input string. This means, in the worst case, when all characters are unique the time complexity of the algorithm will be O(N∗logN)O(N*logN)O(N∗logN) where ‘N’ is the total number of characters in the string.
Space complexity#
