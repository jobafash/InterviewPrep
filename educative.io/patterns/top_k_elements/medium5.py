...
Since we will insert all numbers in a HashMap and a Min Heap, this will take O(N∗logN)O(N*logN)O(N∗logN) where ‘N’ is the total input numbers. While extracting numbers from the heap, in the worst case, we will need to take out ‘K’ numbers. This will happen when we have at least ‘K’ numbers with a frequency of two. Since the heap can have a maximum of ‘N/2’ numbers, therefore, extracting an element from the heap will take O(logN)O(logN)O(logN) and extracting ‘K’ numbers will take O(KlogN)O(KlogN)O(KlogN). So overall, the time complexity of our algorithm will be O(N∗logN+KlogN)O(N*logN + KlogN)O(N∗logN+KlogN).

We can optimize the above algorithm and only push ‘K’ elements in the heap, as in the worst case we will be extracting ‘K’ elements from the heap. This optimization will reduce the overall time complexity to O(N∗logK+KlogK).O(N*logK + KlogK).O(N∗logK+KlogK).
Space complexity#
